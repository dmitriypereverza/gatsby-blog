...
import {
  errorLogger,
  logRequestEndError,
  logRequestEndSuccess,
  logRequestStart,
} from "./logger";
import { appEventEmitter, GlobalEvents } from "../../App";
...
async function makeAndDecodeResponse(
  ...
  try {
    ...
    if (cancelName && cancellations[cancelName]) {
      cancellations[cancelName] = undefined;
    }
...
export function createRequest<DecoderGenericType>(
  ...
  serverDataDecoder?,
  requestConfig?: RequestConfigInterface,
): (data?: RequestOptions) => Promise<void>;
  ...
  return function (
    ...
    return makeRequest(
      ...
      (err: RequestError | string) => {
        if (err === REQUEST_CANCELLED) throw err;
        ...
        if (![400, 401].includes(err.statusCode)) {
          appEventEmitter.emit(
            GlobalEvents.Error,
            new Error(
              "Произошла ошибка при совершении запроса на удаленный сервер: " +
                err.message,
            ),
          );
          logRequestEndError(err);
        }
