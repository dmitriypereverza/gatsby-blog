import { curry, identity, is, memoizeWith } from "ramda";
...
export const getColor = function (color: Colors) {
...
export function backgroundColor(backgroundColor: Colors, opacity?: number) {
  return css`
    background-color: ${opacity
      ? rgba(getColor(backgroundColor), opacity)
...
export const userSelect = (
  value: "text" | "contain" | "none" | "all" | "auto",
) =>
  css`
    user-select: ${value};
  `;
...
export const absoluteCenterByTopLeft = css`
  position: absolute;
  top: 50%;
  left: 50%;
...
export const absoluteCenterByHorizontal = css`
  position: absolute;
  left: 50%;
